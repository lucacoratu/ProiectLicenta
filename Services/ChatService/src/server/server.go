...
func InitServer(address string) error {
	...
	getSubrouter.HandleFunc("/chat/groups/{roomId:[0-9]+}/picture", handlerChat.GetGroupPicture)
	getSubrouter.HandleFunc("/blob/{blobUuid:[0-9a-fA-F\\-]+}", handlerChat.GetDataFromBlob)
