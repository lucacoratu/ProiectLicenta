...
/*
 * This function will get all the bug reports that can be found in the database
 */
func (conn *Connection) GetAllBugReports() (data.BugReports, error) {
	//Prepare the select statement to get all the bug reports
	sqlStmt, err := conn.db.Prepare("SELECT bugreports.Id, bugreportcategories.Name, Description, accounts.DisplayName FROM bugreports INNER JOIN bugreportcategories ON bugreportcategories.ID = bugreports.CategoryId INNER JOIN accounts ON accounts.ID = bugreports.ReportedBy")
	...
	for rows.Next() {
		...
		_ = rows.Scan(&bugReport.ID, &bugReport.Category, &bugReport.Description, &bugReport.ReportedBy)
