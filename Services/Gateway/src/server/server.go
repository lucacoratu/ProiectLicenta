...
func InitServer(address string) error {
	...
	handlerRegister := handlers.NewAccountRegister(serverLogger, svc)
	handlerLogin := handlers.NewAccountLogin(serverLogger, svc)
	handlerFriendRequests := handlers.NewFriendRequest(serverLogger, svc)
	handlerFriend := handlers.NewFriend(serverLogger, svc)
	handlerFeedback := handlers.NewFeedback(serverLogger, svc)
	...
	postRouter.HandleFunc("/account/update/about", handlerFeedback.UpdateAboutMessage)
	postRouter.HandleFunc("/account/cansend/friendrequest", handlerFeedback.CanSendFriendRequest)
