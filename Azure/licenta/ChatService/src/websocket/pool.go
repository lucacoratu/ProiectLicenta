 ...
 */
func NewPool(l logging.ILogger, dbConn database.IConnection, conf data.Configuration) *Pool {
	return &Pool{
...
func (pool *Pool) sendUpdateStatusAccountService(accountId int, newStatus string) error {
	...
	url, err := pool.configuration.GetServiceURL("accountservice")
	if err != nil {
		pool.logger.Error("Cannot get the account service url from config")
		return err
	}

	req, err := http.NewRequest(http.MethodPut, "http://"+url+"/status", bytes.NewBuffer(json))
	if err != nil {
		pool.logger.Error(err.Error())
		return err
	}
