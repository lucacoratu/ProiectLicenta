...
func (fr *Friend) AddFriend(rw http.ResponseWriter, r *http.Request) {
	...
	resp, err = http.Post("http://frequestservice:8083/delete", "application/json", bytes.NewReader(bodyData))
	...
	resp, err = http.Post("http://chatservice:8087/privateroom/create", "application/json", bytes.NewReader(bodyData))
	//Check if an error occured when sending the data to the friendrequest service
	if err != nil {
		//An error occured
		//Log the error
...
func (fr *Friend) GetFriends(rw http.ResponseWriter, r *http.Request) {
	...
	fr.logger.Debug("Friends:", friends)
	...
	for _, friend := range friends {
		...
		resp, _ := http.Post("http://chatservice:8087/privateroom", "application/json", reqBodyReader)
		//Check for errors
		...
		resp, _ = http.Get("http://chatservice:8087/history/" + strconv.FormatInt(roomIdResp.RoomID, 10) + "/lastmessage")
		lastMessage := data.LastMessage{}
		_ = lastMessage.FromJSON(resp.Body)
		fr.logger.Debug("Last message = ", lastMessage)
		if string(lastMessage.MessageText) != "" {
			friendsMessages = append(friendsMessages, data.FriendAndLastMessage{ID: friend.ID, AccountID: friend.AccountID, FriendID: friend.FriendID, RoomID: roomIdResp.RoomID, BefriendDate: friend.BefriendDate, LastMessage: string(lastMessage.MessageText), LastMessageTimestamp: lastMessage.MessageTimestamp})
		} else {
			friendsMessages = append(friendsMessages, data.FriendAndLastMessage{ID: friend.ID, AccountID: friend.AccountID, FriendID: friend.FriendID, RoomID: roomIdResp.RoomID, BefriendDate: friend.BefriendDate, LastMessage: "Start conversation", LastMessageTimestamp: ""})
		}
	}
