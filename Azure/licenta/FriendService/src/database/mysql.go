...
func (conn *Connection) InitializeConnection() error {
	dbUser := "root"
	dbPassword := "root" //Password not required
	dbHost := "willow-db"
	dbName := "willow"
...
func (conn *Connection) AddFriend(accID int, friendID int) error {
	...
	stmtInsert, err := conn.db.Prepare("INSERT INTO friendships (AccountID, FriendID, BefriendDate) VALUEs (?, ?, 0)")
...
func (conn *Connection) ViewFriendships(accID int) (data.Friends, error) {
	...
	stmtSelect, err := conn.db.Prepare("SELECT ID, AccountID, FriendID, BefriendDate FROM friendships WHERE AccountID = ? OR FriendID = ?")
	...
	for rows.Next() {
		...
		err = rows.Scan(&friendRequest.ID, &friendRequest.AccountID, &friendRequest.FriendID, &friendRequest.BefriendDate)
		if err != nil {
