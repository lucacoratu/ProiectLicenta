...
func NewChat(logger logging.ILogger, dbConn *database.Connection, conf data.Configuration) *Chat {
...
func (ch *Chat) GetGroups(rw http.ResponseWriter, r *http.Request) {
	...
	url, err := ch.configuration.GetServiceURL("chatservice")
	if err != nil {
		ch.logger.Error("Cannot get the account service url from config")
		return
	}

	//Forward the request to the chat service
	returnData, err := ch.ForwardRequest("http", url, r)
	if err != nil {
		http.Error(rw, err.Error(), http.StatusInternalServerError)
		return
	}
