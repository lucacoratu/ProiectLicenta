...
func NewFriendRequest(logger logging.ILogger, dbConn *database.Connection, conf data.Configuration) *FriendRequest {
	return &FriendRequest{logger: logger, dbConn: dbConn, configuration: conf}
}
...
func (frReq *FriendRequest) AddFriendRequest(rw http.ResponseWriter, r *http.Request) {
	...
	url, err := frReq.configuration.GetServiceURL("friendrequestservice")
	if err != nil {
		frReq.logger.Error("Cannot get the account service url from config")
		return
	}

	response, err := http.Post("http://"+url+"/add", "application/json", r.Body)
	if err != nil {
...
func (frReq *FriendRequest) DeleteFriendRequest(rw http.ResponseWriter, r *http.Request) {
	...
	response, err := http.Post("http://"+url+"/delete", "application/json", r.Body)
	if err != nil {
		frReq.logger.Error("Cannot send data to FriendRequestService", err.Error())
		jsonError := jsonerrors.JsonError{Message: "Cannot send request to FriendRequestService"}
		rw.WriteHeader(http.StatusInternalServerError)
...
func (frReq *FriendRequest) ViewFriendRequests(rw http.ResponseWriter, r *http.Request) {
	...
	response, err := http.Get("http://" + url + "/view/" + vars["id"])
	if err != nil {
		frReq.logger.Error("Cannot send data to FriendRequestService", err.Error())
		jsonError := jsonerrors.JsonError{Message: "Cannot send request to FriendRequestService"}
		rw.WriteHeader(http.StatusInternalServerError)
...
func (frReq *FriendRequest) ViewSentFriendRequests(rw http.ResponseWriter, r *http.Request) {
	...
	vars := mux.Vars(r)
	frReq.logger.Debug("Id received is", vars["id"])
	response, err := http.Get("http://" + url + "/viewsent/" + vars["id"])
	if err != nil {
		frReq.logger.Error("Cannot send data to FriendRequestService", err.Error())
		jsonError := jsonerrors.JsonError{Message: "Cannot send request to FriendRequestService"}
		rw.WriteHeader(http.StatusInternalServerError)
...
func (frReq *FriendRequest) GetFriendsRecommendations(rw http.ResponseWriter, r *http.Request) {
	...
	for _, user := range users {
		...
		response, err := http.Post("http://"+url+"/arefriends", "application/json", res)
