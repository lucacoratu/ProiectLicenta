 ...
 */
func NewAccountLogin(l logging.ILogger, conf data.Configuration) *AccountLogin {
	return &AccountLogin{logger: l, configuration: conf}
}
...
func (login *AccountLogin) LoginIntoAccount(rw http.ResponseWriter, r *http.Request) {
	...
	url, err := login.configuration.GetServiceURL("accountservice")
	if err != nil {
		login.logger.Error("Cannot get the account service url from config")
		return
	}

	//Forward the request to the Account Service /login
	response, err := http.Post("http://"+url+"/login", "application/json", r.Body)
